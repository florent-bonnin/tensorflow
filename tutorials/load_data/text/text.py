...
print(binary_model.predict(tf.constant(['How do you sort a list?'])))
...
class PreprocessTextLayer(tf.keras.layers.Layer):
    def __init__(self, tokenizer, vocab_table):
        super().__init__()
    ...
    def call(self, text):
        ...
        return self.vocab_table(tokens)

preprocess_text = PreprocessTextLayer(tokenizer, vocab_table)
...
export_model.export('export.tf')

loaded = tf.saved_model.load('export.tf')

print(export_model(tf.constant(['The field bristled with the long and deadly spears which they bore.'])).numpy())

print(loaded.serve(tf.constant(['The field bristled with long and deadly spears which they bore.'])).numpy())
